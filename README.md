<h1 align="center">Hi, I'm Arsi — DevOps & Backend Explorer</h1>

```yaml
  status: learning C, Bash, Linux, CI/CD, Docker
  focus: backend logic, infrastructure, automation
  motto: Building solid skills with limited time and unlimited persistence

🚀 About Me

I’m a self-taught developer to DevOps/backend engineering.

Currently studying at School 21, where I:
	•	Dive deep into low-level programming (C)
	•	Learn system thinking through Bash & Linux
	•	Practice peer-reviewed, project-based education

I’m actively building side projects to apply:
	•	Backend APIs (Node.js, Express)
	•	Infrastructure as Code (Terraform — in progress)
	•	CI/CD pipelines (GitHub Actions)

🛠 Projects
	•	life_pong – terminal Pong x Game of Life in C
	•	matrix – data visualization heatmap in Python
	•	p2p_vers01 – fullstack React + Express + Docker demo
	•	p2pNetwork – fullstack app with Drizzle ORM & modern web stack demo

🧠 What I’m Learning
	•	DevOps workflows
	•	Cloud infrastructure
	•	Low-level programming and backend patterns

🌍 Let’s Connect
  •     Mail  arildfk@gmail.com	
  •	Instagram  https://www.instagram.com/arildmt?igsh=MTZwZ2ZwaDhwajRkcg%3D%3D&utm_source=qr

## Collaborative Projects (built with peers from School 21)

### `p2p_vers01`
**Acknowledgements:**  
This project was developed with support from fellow students at School 21.  
Special thanks to those who helped during brainstorming and code reviews.

### `p2pNetwork`
**Contributors:**  
This project is a result of collaborative learning inside School 21’s peer-to-peer system.  
Some architecture and tooling decisions were shaped during teamwork and technical discussions with classmates.

### `p2p-main`
**Note:**  
This is an experimental frontend scaffold, initially built during study sessions with peers at School 21.
